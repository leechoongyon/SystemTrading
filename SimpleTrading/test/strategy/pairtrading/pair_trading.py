...
from simple.strategy.pairtrading.common.pairtrading_common import getData, \
    symbolToPath
...
if __name__ == '__main__':
    ...
    for stockCd in stockCds:    
        ...
        rows = data_crawler.getHistoricalData(stockCd, start, end, 
                                              int(pageNum), int(totalPageNum))
        df = pd.DataFrame(rows, columns=["Stock_Cd", "Date", "Open", "High", 
                                         "Low", "Close", "Volume", 
                                         "Adj Close"])
    ...
    for stockCd in stockCds:
        df_temp = pd.read_csv(symbolToPath(stockCd, 
                              ...
                              na_values=['nan'])
        df_temp = df_temp.rename(columns={'Adj Close': stockCd})
        df = df.join(df_temp)
